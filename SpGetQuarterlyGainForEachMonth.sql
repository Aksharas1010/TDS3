-- Modify the stored procedure to accept a client ID parameter  
CREATE PROCEDURE SpGetQuarterlyGainForEachMonth  
    @todate VARCHAR(20),  
    @Client INT -- Add a parameter for the client ID  
AS  
BEGIN  
    SET NOCOUNT ON;  
    ...
    WITH FirstItemOfMonth AS (  
        ...
        WHERE  
            ...
            AND Client = @Client -- Filter by client ID  
        GROUP BY  
    ...
    SELECT  
        ...
        ISNULL((SELECT OpeningBalST FROM Tax_Daily_Profit_Summary WHERE TransSaleDate = F.FirstItemDate AND Client = @Client), 0.00) AS OpeningBalanceST,  
        ISNULL((SELECT ClosingBalST FROM Tax_Daily_Profit_Summary WHERE TransSaleDate = L.LastItemDate AND Client = @Client), 0.00) AS ClosingBalanceST,  
        COALESCE(M.MonthProfitSumST, 0.00) AS STGain,  
        COALESCE(T.MonthTaxSumST, 0.00) AS STTaxSum,  
        ISNULL((SELECT OpeningBalLT FROM Tax_Daily_Profit_Summary WHERE TransSaleDate = F.FirstItemDate AND Client = @Client), 0.00) AS OpeningBalanceLT,  
        ISNULL((SELECT ClosingBalLT FROM Tax_Daily_Profit_Summary WHERE TransSaleDate = L.LastItemDate AND Client = @Client), 0.00) AS ClosingBalanceLT,  
        COALESCE((SELECT MAX(ST_TaxPercentage) FROM Tax_Daily_Profit_Summary WHERE TransSaleDate = L.LastItemDate AND Client = @Client), 0.00) AS STTaxPercentage,  
        COALESCE((SELECT MAX(LT_TaxPercentage) FROM Tax_Daily_Profit_Summary WHERE TransSaleDate = L.LastItemDate AND Client = @Client), 0.00) AS LTTaxPercentage,  
